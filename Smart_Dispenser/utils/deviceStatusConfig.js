...
export const getBatteryDeviceStatus = (device) => {
  ...
  // Check for battery percentage thresholds
  if (
    device.battery_percentage !== null &&
    device.battery_percentage !== undefined
  ) {
    if (device.battery_percentage <= 10) {
      return "battery_critical";
    }
...
export const getDeviceStatusConfig = (status, isActive, isDark) => {
  ...
  if (Array.isArray(status)) {
    ...
    normalizedStatus = status?.toLowerCase() || "unknown";
  }
  ...
  switch (normalizedStatus) {
    ...
    case "power_off":
      return {
        color: isDark ? "#757575" : "#757575", // Gray for Power Off
        bgLight: "#F5F5F5",
        bgDark: isDark ? "#232323" : "rgba(117, 117, 117, 0.15)",
        icon: "power",
        text: "Power Off",
        gradient: ["#BDBDBD", "#757575"],
        shadowColor: "#757575",
        priority: 70,
      };
