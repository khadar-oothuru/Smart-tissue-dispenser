...
def get_devices(request):
    ...
    paginator.page_size = 20    # Select all fields including metadata if it exists
    ...
    if hasattr(Device, 'tissue_type'):
        fields_to_select.append('tissue_type')
    if hasattr(Device, 'meter_capacity'):
        fields_to_select.append('meter_capacity')
    if hasattr(Device, 'refer_value'):
        fields_to_select.append('refer_value')
...
@permission_classes([IsAuthenticated])
def device_detail(request, pk=None, device_id=None):
    try:
        if pk is not None:
            # Try to get by primary key (integer ID)
            if isinstance(pk, str) and not pk.isdigit():
                # If pk is a string but not a digit, treat it as device_id
                device = Device.objects.get(device_id=pk)
            else:
                device = Device.objects.get(pk=pk)
        elif device_id is not None:
            # Get by device_id (string)
            device = Device.objects.get(device_id=device_id)
        else:
            return Response({'error': 'No device identifier provided'}, status=status.HTTP_400_BAD_REQUEST)
...
@swagger_auto_schema(
    ...
    request_body=openapi.Schema(
        ...
        properties={
            ...
            'tissue_type': openapi.Schema(type=openapi.TYPE_STRING, description='Type of tissue: hand_towel or toilet_paper', enum=['hand_towel', 'toilet_paper']),
            'meter_capacity': openapi.Schema(type=openapi.TYPE_INTEGER, description='Meter capacity (positive integer, e.g., 500, 1000, 1500)'),
...
def register_device_via_wifi(request):
    ...
    device_data = {
        ...
        'tissue_type': request.data.get('tissue_type', 'hand_towel'),
        'meter_capacity': request.data.get('meter_capacity', 500),
    }
    ...
    try:
      ...
      # Validate meter_capacity
    try:
        meter_capacity = int(device_data['meter_capacity'])
        if meter_capacity <= 0:
            meter_capacity = 500  # default if invalid
        device_data['meter_capacity'] = meter_capacity
    except (ValueError, TypeError):
        device_data['meter_capacity'] = 500
    
    # Validate tissue_type
    if device_data['tissue_type'] not in ['hand_towel', 'toilet_paper']:
        device_data['tissue_type'] = 'hand_towel'
