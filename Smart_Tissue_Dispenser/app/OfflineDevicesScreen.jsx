...
export default function OfflineDevicesScreen() {
  ...
  // Filter for offline devices: current_status 'offline' but NOT power off or no power devices
  // Include battery off devices in offline category
  const offlineDevices = useMemo(() => {
    ...
    const isBatteryOff = (device) => {
      return (
        device.battery_percentage === 0 ||
        device.battery_off === 1 ||
        device.battery_off_count > 0 ||
        (device.current_status &&
          String(device.current_status).toLowerCase() === "battery_off")
      );
    };

    return mergedDevices.filter((device) => {
      const status = (device.current_status || "").toLowerCase();
      const powerStatus = device.pwrstatus || device.power_status;
      const isPowerOffDevice = isPowerOff(powerStatus);
      const isNoPowerDevice = isNoPower(powerStatus);
      const isBatteryOffDevice = isBatteryOff(device);

      // Include:
      // 1. Offline devices (but not power off or no power)
      // 2. Battery off devices
      // Exclude: Power off devices and No power devices (they have their own screens)
      return (
        (status === "offline" && !isPowerOffDevice && !isNoPowerDevice) ||
        isBatteryOffDevice
      );
    });
  }, [mergedDevices]);
  ...
  const sortedDevices = useMemo(() => {
    const getDevicePriority = (device) => {
      ...
      // Battery off devices get highest priority
      if (
        device.battery_percentage === 0 ||
        device.battery_off === 1 ||
  ...
  return (
    <ScreenWrapper>
      <View style={{ flex: 1, backgroundColor: themeColors.background }}>
        ...
        />
        <DeviceHeader
          title="Offline & Battery Off Devices"
          subtitle={`${filteredDevices.length} device${
            filteredDevices.length === 1 ? "" : "s"
          } offline or with dead battery`}
        ...
        <ScrollView>
          {filteredDevices.length === 0 ? (
            ...
            >
              ...
              >
                {searchTerm.trim()
                  ? `No offline devices found for "${searchTerm}"`
                  : "All devices are online with sufficient battery"}
              </Text>
            </View>
          ) : (
