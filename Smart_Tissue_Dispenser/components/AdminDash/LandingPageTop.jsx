...
export default function LandingPageTop({
  stats,
  summaryData,
  realtimeStatus = [], // Add realtimeStatus for battery off calculation
  onRefresh,
  isLoading = false,
  onPressTotalDevices,
}) {
  ...
  const handlePressNoPower = () => {
    navigation.navigate("NoPowerScreen");
  };
  ...
  // Calculate no power devices (pwr_sts = "no")
  const noPowerDevices = useMemo(() => {
    const { noPowerCount } = getBatteryAndPowerAlertCounts(realtimeStatus);
    return noPowerCount;
  }, [realtimeStatus]);
      ...
      >
        ...
        >
          ...
          <View style={styles.cardContent}>
            ...
            {/* Main Stats Layout - Total Device card full width above, then 3 cards below */}
            <View
              style={{
            ...
            >
              {/* Total Dispensers - Full Width Card */}
              <Pressable
                style={{ width: "100%" }}
              ...
              {/* Active, Offline, Battery Off - Three Card Row */}
              <View
                style={{
                  flexDirection: "row",
                  alignItems: "stretch",
